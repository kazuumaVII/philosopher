...
/*   Updated: 2021/03/09 13:24:09 by frfrance         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../philo.h"

void					ft_sleep(unsigned int time, t_all *all)
{
	long unsigned int	finish;

	finish = get_time(all->philo->time) + time;
	while (get_time(all->philo->time) < finish)
		usleep(1);
}
